...
class DiTwDDTHead(nn.Module):
    def __init__(
        ...
        if K > 0:
            ...
            self.special_pos_embed_s = nn.Parameter(
                torch.zeros(1, K, self.encoder_hidden_size), requires_grad=False)
            ...
            self.special_pos_embed_x = nn.Parameter(
                torch.zeros(1, K, self.decoder_hidden_size), requires_grad=False)
    ...
    def initialize_weights(self, xavier_uniform_init: bool = False):
        ...
        if self.num_special_tokens > 0:
            ...
            nn.init.constant_(self.special_pos_embed_s, 0)
            nn.init.constant_(self.special_pos_embed_x, 0)
    ...
    def forward(self, x, t, total_view, camera_embedding, s=None, mask=None, pag_mode=False, pag_layer_idx=None, **kwargs):
        ...
        if s is None:
            ...
            if source_condition is not None and self.source_condition_mode == "l1_as_cond":
                ...
                s_ref = self._embed_fp32(self.s_embedder_ref, ei_ref)
                if cond_num < total_view:
                    s_tgt = self._embed_fp32(self.s_embedder_tgt, ei_tgt)
                    s = torch.cat([rearrange(s_ref, "(b v) n d -> b v n d", v=cond_num),
                                   rearrange(s_tgt, "(b v) n d -> b v n d", v=total_view - cond_num)], dim=1)
                else:
                    s = rearrange(s_ref, "(b v) n d -> b v n d", v=cond_num)
                ...
                s_ref = self._embed_fp32(self.s_embedder_ref, xp_ref)
                if cond_num < total_view:
                    s_tgt = self._embed_fp32(self.s_embedder_tgt, xp_tgt)
                    s = torch.cat([rearrange(s_ref, "(b v) n d -> b v n d", v=cond_num),
                                   rearrange(s_tgt, "(b v) n d -> b v n d", v=total_view - cond_num)], dim=1)
                else:
                    s = rearrange(s_ref, "(b v) n d -> b v n d", v=cond_num)
                ...
                s = self._embed_fp32(self.s_embedder, x_patches)
            ...
            if has_cls:
                s_cls = self._embed_fp32(self.cls_embedder, x_cls).unsqueeze(1)
            ...
            cam_emb = self._embed_fp32(self.camera_embedder, camera_embedding)
            ...
            if has_special:
                ...
                special_s = self._embed_fp32(self.special_s_embedder, x_special)   # (BV, K, enc_hidden)
        ...
        if self.architecture_mode == "new":
            ...
            x_toks_ref = self._embed_fp32(self.x_embedder_ref, xp_ref)
            if cond_num < total_view:
                x_toks_tgt = self._embed_fp32(self.x_embedder_tgt, xp_tgt)
                ...
                x_toks = rearrange(x_toks_ref, "(b v) n d -> b v n d", v=cond_num)
            x_toks = rearrange(x_toks, "b v n d -> (b v) n d")
        else:
            # OLD Architecture (Single Embedder)
            x_toks = self._embed_fp32(self.x_embedder, x_patches)
        ...
        if has_cls:
            x_cls_tok = self._embed_fp32(self.x_cls_embedder, x_cls).unsqueeze(1)
        ...
        if has_special:
            special_x = self._embed_fp32(self.special_x_embedder, x_special)   # (BV, K, dec_hidden)
