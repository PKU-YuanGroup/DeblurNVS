...
class DA3EncoderDirect(nn.Module):
    ...
    def __init__(
        ...
        # The open-source demo is fully local: DA3 weights must already exist on disk.
        if not Path(pretrained_path).exists():
            raise FileNotFoundError(f"Local DA3 checkpoint directory not found: {pretrained_path}")
        da3 = DepthAnything3.from_pretrained(str(pretrained_path), local_files_only=True)
