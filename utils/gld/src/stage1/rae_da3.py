...
class RAE_DA3(nn.Module):
    ...
    def __init__(
        ...
        load_dpt_decoder_override: bool = True,
        noise_tau: float = 0.0,
        ...
        if da3_weights_path is not None:
            decoder_override_path = dpt_decoder_path if bool(load_dpt_decoder_override) else None
            self._init_dpt_decoder(da3_weights_path, decoder_override_path)
            
        # Initialize MAE decoder if weight is provided
        self.mae_decoder = None
        if mae_weight is not None or decoder_config_path is not None:
            ...
            if mae_weight is not None:
                ...
                print(f"Initializing MAE decoder architecture from config {decoder_config_path}")
    ...
    def _init_dpt_decoder(self, da3_weights_path: str, dpt_decoder_path: Optional[str] = None):
        ...
        if dpt_decoder_path is not None:
            ...
            print("Using official DA3 dual-head weights only (no DPT decoder override).")
