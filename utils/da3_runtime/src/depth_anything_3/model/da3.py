...
from src.depth_anything_3.cfg import create_object
from src.depth_anything_3.model.utils.transform import pose_encoding_to_extri_intri
from src.depth_anything_3.utils.alignment import (
...
from src.depth_anything_3.utils.geometry import affine_inverse, as_homogeneous, map_pdf_to_opacity
...
class DepthAnything3Net(nn.Module):
    ...
    def __init__(self, net, head, cam_dec=None, cam_enc=None, gs_head=None, gs_adapter=None):
        ...
        """
        super().__init__()
        self.enable_mono_sky_estimation = True
    ...
    def forward(
        ...
        export_feat_layers: list[torch.Tensor] | None = [],
        infer_gs: bool = False,
    ...
    def _process_mono_sky_estimation(
        ...
        if not bool(getattr(self, "enable_mono_sky_estimation", True)):
            return output
    ...
    def _process_camera_estimation(
        self, feats, H: int, W: int, output: Dict[str, torch.Tensor]
    ) -> Dict[str, torch.Tensor]:
    ...
    def _extract_auxiliary_features(
        self, feats, feat_layers: list[int], H: int, W: int
    ) -> Dict[str, torch.Tensor]:
...
class NestedDepthAnything3Net(nn.Module):
    ...
    def __init__(self, anyview: Union[DictConfig, dict], metric: Union[DictConfig, dict]):
        """
        ...
        # Convert dict to DictConfig if needed (when loaded from HuggingFace)
        if not isinstance(anyview, DictConfig):
            anyview = _wrap_cfg(anyview)
        if not isinstance(metric, DictConfig):
            metric = _wrap_cfg(metric)
