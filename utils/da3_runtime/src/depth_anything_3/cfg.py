...
def load_config(path: str, argv: List[str] = None) -> Union[DictConfig, ListConfig]:
    ...
    if "." in path and "/" not in path and not path.endswith(".yaml"):
        ...
        # Handle paths starting with src.depth_anything_3
        if len(path_parts) >= 3 and path_parts[0] == "src" and path_parts[1] == "depth_anything_3":
            # Skip 'src' and 'depth_anything_3', start from the configs directory
            path_parts = path_parts[2:]
            # Start from the depth_anything_3 directory (where cfg.py is located)
            config_path = Path(__file__).resolve().parent
        else:
            # Original behavior: skip first part, start from parent directory
